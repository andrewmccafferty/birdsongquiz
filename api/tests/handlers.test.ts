...
describe("getSpeciesList tests", () => {
  ...
  describe("when querying by region", () => {
    ...
    it("should return species list for valid region", async () => {
      ...
      const response = await getSpeciesList(mockApiGatewayEvent(null, { region: TEST_REGION }))
      
      expect(loadSpeciesListForRegion).toHaveBeenCalledWith(TEST_REGION)
      expect(loadSpeciesListById).not.toHaveBeenCalled()
      expect(response.statusCode).toEqual(200)
      expect(response.headers?.['Cache-Control']).toEqual(CACHE_CONTROL_HEADER_MAX_AGE_YEAR)
    ...
    it("should return 404 when no species list found for region", async () => {
      ...
      expect(JSON.parse(response.body)).toEqual({ message: 'No species list found for region' })
    })
  })
  ...
  describe("when querying by listId", () => {
    ...
    it("should return species list for valid listId", async () => {
      ...
      const response = await getSpeciesList(mockApiGatewayEvent(null, { listId: TEST_LIST_ID }))
      
      expect(loadSpeciesListById).toHaveBeenCalledWith(TEST_LIST_ID.toLowerCase())
    ...
    it("should convert listId to lowercase before querying", async () => {
      ...
      await getSpeciesList(mockApiGatewayEvent(null, { listId: UPPERCASE_LIST_ID }))
      
      expect(loadSpeciesListById).toHaveBeenCalledWith(UPPERCASE_LIST_ID.toLowerCase())
    })
    ...
    it("should return 404 when no species list found for listId", async () => {
      ...
      expect(response.statusCode).toEqual(404)
      expect(JSON.parse(response.body)).toEqual({ message: 'No species list found for given id' })
    })
  })
  ...
  describe("response headers", () => {
    it("should include CORS headers", async () => {
      ...
      const response = await getSpeciesList(mockApiGatewayEvent(null, { region: "GB" }))
