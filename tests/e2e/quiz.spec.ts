...
test(
  ...
  async ({ page }) => {
    ...
    await page
      .getByRole("option", { name: "Garden Warbler", exact: true })
      .click();
...
test(
  "Run quiz with Australian species",
  { tag: RUN_IN_PROD_TAG },
  async ({ page }) => {
    await page.goto(FRONTEND_URL);
    ...
    await page
      .getByRole("combobox", { name: "Start typing to choose a" })
      .fill("White-throated Hon");
    await page
      .getByRole("option", { name: "White-throated Honeyeater" })
      .click();
    await page.locator('input[type="text"]').fill("yellow-tuft");
    await page
      .getByRole("option", { name: "Yellow-tufted Honeyeater", exact: true })
      .click();
    await page.getByTestId("finish-selection").click();
    await page
      .getByRole("button", { name: "Yellow-tufted Honeyeater" })
      .click();
    await page.getByTestId("next-clip").click();
    await page
      .getByRole("button", { name: "White-throated Honeyeater" })
      .click();
    await page.getByTestId("next-clip").click();
    await page.getByTestId("reset").click();
  }
